...
class DB5DGLDataModule(LightningDataModule):
    ...
    # Dataset partition instantiation
    db5_train = None
    db5_val = None
    db5_test = None
    ...
    def __init__(self, data_dir: str, batch_size: int, num_dataloader_workers: int, knn: int,
        ...
        self.percent_to_use = percent_to_use  # Fraction of DB5 dataset to use
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.db5_train = DB5DGLDataset(mode='train', raw_dir=self.data_dir, knn=self.knn, self_loops=self.self_loops,
                                       percent_to_use=self.percent_to_use, process_complexes=self.process_complexes,
                                       input_indep=self.input_indep)
        self.db5_val = DB5DGLDataset(mode='val', raw_dir=self.data_dir, knn=self.knn, self_loops=self.self_loops,
                                     percent_to_use=self.percent_to_use, process_complexes=self.process_complexes,
                                     input_indep=self.input_indep)
        self.db5_test = DB5DGLDataset(mode='test', raw_dir=self.data_dir, knn=self.knn, self_loops=self.self_loops,
                                      percent_to_use=self.percent_to_use, process_complexes=self.process_complexes,
                                      input_indep=self.input_indep)

    def train_dataloader(self) -> DataLoader:
        return DataLoader(self.db5_train, batch_size=self.batch_size, shuffle=True,
                          num_workers=self.num_dataloader_workers, collate_fn=lambda x: x, pin_memory=True)
