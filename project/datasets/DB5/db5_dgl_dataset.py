...
from project.utils.training_utils import construct_filenames_frame_txt_filenames, build_filenames_frame_error_message, process_complex_into_dict, zero_out_complex_features
...
class DB5DGLDataset(DGLDataset):
    ...
    def __init__(self,
        ...
        # Process any unprocessed examples prior to using the dataset
        self.process()
    ...
    def process(self):
        ...
        if self.process_complexes:
            ...
            for (i, raw_path) in self.filenames_frame.iterrows():
                raw_filepath = os.path.join(self.root, f'{os.path.splitext(raw_path[0])[0]}.dill')
                ...
                if not os.path.exists(processed_filepath):
                    ...
                    process_complex_into_dict(raw_filepath, processed_filepath, self.knn, self.self_loops, False)
